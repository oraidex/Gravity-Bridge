...
use gravity_utils::types::{
    event_signatures::*, Erc20DeployedEvent, EthereumEvent, LogicCallExecutedEvent,
    SendERC721ToCosmosEvent, SendToCosmosEvent, TransactionBatchExecutedEvent, ValsetUpdatedEvent,
};
    ...
    while current_block.clone() > previous_block {
        ...
        if contract_type == ContractType::GravityERC721 {
            ...
            while ind < callback_chains.len() {
                ...
                if let Some(block) =
                    callback(last_event_nonce.clone(), our_cosmos_address, &web3_event)
                {
