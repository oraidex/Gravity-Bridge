...
func NewInternalOutgoingTransferTx(
	...
	if err != nil {
		return nil, errorsmod.Wrap(err, "invalid evm destination")
	}
...
func NewInternalOutgingTxBatchFromExternalBatch(batch OutgoingTxBatch) (*InternalOutgoingTxBatch, error) {
	...
	if err != nil {
		return nil, errorsmod.Wrap(err, "invalid evm address")
	}
...
func (i InternalOutgoingTxBatch) TotalValue(evmChainPrefix string) sdk.Coin {
	var totalAmount sdk.Coin = sdk.Coin{}
	for _, tx := range i.Transactions {
		if totalAmount.Denom == "" { // Initialize an empty coin
			totalAmount = tx.Erc20Fee.GravityCoin(evmChainPrefix)
		} else {
			totalAmount = totalAmount.Add(tx.Erc20Fee.GravityCoin(evmChainPrefix))
		}
		totalAmount = totalAmount.Add(tx.Erc20Token.GravityCoin(evmChainPrefix))
	}
...
func (i InternalOutgoingLogicCall) TotalValue(evmChainPrefix string) sdk.Coins {
	...
	return totalAmount.ToCoins(evmChainPrefix)
}
