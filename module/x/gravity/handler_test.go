...
func TestMsgSendToCosmosClaim(t *testing.T) {
	...
	for _, v := range keeper.OrchAddrs {
		...
		require.NoError(t, err)
		a := input.GravityKeeper.GetAttestation(ctx, uint64(1), hash, types.GravityContractNonce)
		require.NotNil(t, a)
...
func TestMsgSendToCosmosClaimSpreadVotes(t *testing.T) {
	...
	for i := range []int{0, 1, 2} {
		...
		hash, err := ethClaim.ClaimHash()
		require.NoError(t, err)
		a1 := input.GravityKeeper.GetAttestation(ctx, myNonce, hash, types.GravityContractNonce)
		require.NotNil(t, a1)
	...
	hash, err := ethClaim.ClaimHash()
	require.NoError(t, err)
	a2 := input.GravityKeeper.GetAttestation(ctx, myNonce, hash, types.GravityContractNonce)
	require.NotNil(t, a2)
	...
	hash, err = ethClaim.ClaimHash()
	require.NoError(t, err)
	a3 := input.GravityKeeper.GetAttestation(ctx, myNonce, hash, types.GravityContractNonce)
	require.NotNil(t, a3)
