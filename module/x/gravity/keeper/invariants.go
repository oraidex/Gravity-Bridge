...
func ValidateStore(ctx sdk.Context, evmChainPrefix string, k Keeper) error {
	...
	lastObservedEventNonce := k.GetLastObservedEventNonce(ctx, types.GravityContractNonce, evmChainPrefix)
	lastObservedEthereumClaimHeight := uint64(0) // used later to validate ethereum claim height
	// OracleAttestationKey
	k.IterateAttestations(ctx, types.GravityContractNonce, evmChainPrefix, false, func(key []byte, att types.Attestation) (stop bool) {
...
func CheckBatches(ctx sdk.Context, evmChainPrefix string, k Keeper) error {
	...
	var err error = nil
	k.IterateClaims(ctx, types.GravityContractNonce, evmChainPrefix, true, types.CLAIM_TYPE_BATCH_SEND_TO_ETH, func(key []byte, att types.Attestation, claim types.EthereumClaim) (stop bool) {
...
func CheckValsets(ctx sdk.Context, evmChainPrefix string, k Keeper) error {
	...
	k.IterateClaims(ctx, types.GravityContractNonce, evmChainPrefix, true, types.CLAIM_TYPE_VALSET_UPDATED, func(key []byte, att types.Attestation, claim types.EthereumClaim) (stop bool) {
		if !att.Observed {
