...
func (a AttestationHandler) handleSendToCosmos(ctx sdk.Context, claim types.MsgSendToCosmosClaim) error {
	...
	if err != nil {
		...
		if er != nil {
			return errorsmod.Wrapf(er, "Could not compute ClaimHash for claim %v: %v", claim, er)
		}
	...
	if errEvmChainSender != nil {
		hash, er := claim.ClaimHash()
		if er != nil {
			return errorsmod.Wrapf(er, "Unable to log error %v, could not compute ClaimHash for claim %v: %v", errEvmChainSender, claim, er)
		}
		...
		return errorsmod.Wrap(errTokenAddress, "invalid evn chain sender on claim")
	}
