...
func (k Keeper) Attest(
	...
	// in the endBlocker.
	lastEventNonce := k.GetLastEventNonceByValidator(ctx, valAddr, nonceSource)
	...
	att := k.GetAttestation(ctx, claim.GetEventNonce(), hash, nonceSource)
	...
	if ethClaim.GetEthBlockHeight() == claim.GetEthBlockHeight() {
		...
		k.SetLastEventNonceByValidator(ctx, valAddr, claim.GetEventNonce(), nonceSource)
...
func (k Keeper) TryAttestation(ctx sdk.Context, att *types.Attestation, nonceSource types.NonceSource) {
	...
	if !att.Observed {
		...
		for _, validator := range att.Votes {
			...
			if attestationPower.GT(requiredPower) {
				lastEventNonce := k.GetLastObservedEventNonce(ctx, nonceSource)
				...
				k.processAttestation(ctx, att, claim, nonceSource)
				k.emitObservedEvent(ctx, att, claim, nonceSource)
...
func (k Keeper) processAttestation(ctx sdk.Context, att *types.Attestation, claim types.EthereumClaim, nonceSource types.NonceSource) {
	hash, err := claim.ClaimHash()
	if err != nil {
		panic("unable to compute claim hash")
	}
	...
	if err := k.AttestationHandler.Handle(xCtx, *att, claim); err != nil { // execute with a transient storage
		...
		k.logger(ctx).Error("attestation failed",
			...
			"id", types.GetAttestationKey(claim.GetEventNonce(), hash, nonceSource),
...
// consensus state.
func (k Keeper) emitObservedEvent(ctx sdk.Context, att *types.Attestation, claim types.EthereumClaim, nonceSource types.NonceSource) {
	hash, err := claim.ClaimHash()
	if err != nil {
	...
	ctx.EventManager().EmitTypedEvent(
		&types.EventObservation{
			...
			AttestationId:   string(types.GetAttestationKey(claim.GetEventNonce(), hash, nonceSource)),
			Nonce:           fmt.Sprint(claim.GetEventNonce()),
		},
	)
}
...
func (k Keeper) SetAttestation(ctx sdk.Context, eventNonce uint64, claimHash []byte, att *types.Attestation, nonceSource types.NonceSource) {
	store := ctx.KVStore(k.storeKey)
	aKey := types.GetAttestationKey(eventNonce, claimHash, nonceSource)
	store.Set(aKey, k.cdc.MustMarshal(att))
}

// GetAttestation return an attestation given a nonce
func (k Keeper) GetAttestation(ctx sdk.Context, eventNonce uint64, claimHash []byte, nonceSource types.NonceSource) *types.Attestation {
	store := ctx.KVStore(k.storeKey)
	aKey := types.GetAttestationKey(eventNonce, claimHash, nonceSource)
	bz := store.Get(aKey)
	if len(bz) == 0 {
...
func (k Keeper) DeleteAttestation(ctx sdk.Context, att types.Attestation, nonceSource types.NonceSource) {
	...
	store.Delete(types.GetAttestationKey(claim.GetEventNonce(), hash, nonceSource))
}
...
func (k Keeper) GetAttestationMapping(ctx sdk.Context, nonceSource types.NonceSource) (attestationMapping map[uint64][]types.Attestation, orderedKeys []uint64) {
	attestationMapping = make(map[uint64][]types.Attestation)
	k.IterateAttestations(ctx, nonceSource, false, func(_ []byte, att types.Attestation) bool {
		claim, err := k.UnpackAttestationClaim(&att)
		if err != nil {
...
func (k Keeper) IterateAttestations(ctx sdk.Context, nonceSource types.NonceSource, reverse bool, cb func(key []byte, att types.Attestation) (stop bool)) {
	...
	case types.ERC721ContractNonce:
		keyPrefix = types.OracleERC721AttestationKey
	default:
...
func (k Keeper) IterateClaims(ctx sdk.Context, nonceSource types.NonceSource, reverse bool, claimType types.ClaimType, cb func(key []byte, att types.Attestation, claim types.EthereumClaim) (stop bool)) {
	...
	k.IterateAttestations(ctx, nonceSource, reverse, func(key []byte, att types.Attestation) bool {
...
func (k Keeper) GetMostRecentAttestations(ctx sdk.Context, nonceSource types.NonceSource, limit uint64) []types.Attestation {
	attestationMapping, keys := k.GetAttestationMapping(ctx, nonceSource)
...
func (k Keeper) GetLastObservedEventNonce(ctx sdk.Context, nonceSource types.NonceSource) uint64 {
	store := ctx.KVStore(k.storeKey)

	var key []byte
	switch nonceSource {
	case types.GravityContractNonce:
		key = types.LastObservedEventNonceKey
	case types.ERC721ContractNonce:
		key = types.LastERC721EventNonceByValidatorKey
	default:
...
func (k Keeper) setLastObservedEventNonce(ctx sdk.Context, nonce uint64, nonceSource types.NonceSource) {
	store := ctx.KVStore(k.storeKey)
	last := k.GetLastObservedEventNonce(ctx, nonceSource)
	...
	switch nonceSource {
	case types.GravityContractNonce:
		nonceKey = types.LastObservedEventNonceKey
	case types.ERC721ContractNonce:
		nonceKey = types.LastERC721EventNonceByValidatorKey
	default:
		panic("invalid nonce source")
	}
...
func (k Keeper) GetLastEventNonceByValidator(ctx sdk.Context, validator sdk.ValAddress, nonceSource types.NonceSource) uint64 {
	if err := sdk.VerifyAddressFormat(validator); err != nil {
		panic(sdkerrors.Wrap(err, "invalid validator address"))
	}
	store := ctx.KVStore(k.storeKey)
	bytes := store.Get(types.GetLastEventNonceByValidatorKey(validator, nonceSource))
...
func (k Keeper) SetLastEventNonceByValidator(ctx sdk.Context, validator sdk.ValAddress, nonce uint64, nonceSource types.NonceSource) {
	if err := sdk.VerifyAddressFormat(validator); err != nil {
		panic(sdkerrors.Wrap(err, "invalid validator address"))
	}
	store := ctx.KVStore(k.storeKey)
	store.Set(types.GetLastEventNonceByValidatorKey(validator, nonceSource), types.UInt64Bytes(nonce))
}
