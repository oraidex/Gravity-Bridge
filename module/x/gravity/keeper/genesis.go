...
func initBridgeDataFromGenesis(ctx sdk.Context, k Keeper, data types.EvmChainData) {
	...
	for _, att := range data.Attestations {
		...
		k.SetAttestation(ctx, types.GravityContractNonce, claim.GetEvmChainPrefix(), claim.GetEventNonce(), hash, &att)
	}
	...
	for _, att := range data.Attestations {
		...
		for _, vote := range att.Votes {
			...
			last := k.GetLastEventNonceByValidator(ctx, types.GravityContractNonce, evmChainPrefix, val)
			if claim.GetEventNonce() > last {
				k.SetLastEventNonceByValidator(ctx, types.GravityContractNonce, evmChainPrefix, val, claim.GetEventNonce())
			}
		}
	}
...
func ExportGenesis(ctx sdk.Context, k Keeper) types.GenesisState {
	...
	for ci, evmChain := range chains {
		...
		attmap, attKeys := k.GetAttestationMapping(ctx, types.GravityContractNonce, evmChain.EvmChainPrefix)
		erc721AttMap, erc721AttKeys := k.GetAttestationMapping(ctx, types.ERC721ContractNonce, evmChain.EvmChainPrefix)
		...
		erc721PendingForwards := k.PendingERC721IbcAutoForwards(ctx, evmChain.EvmChainPrefix, 0)
