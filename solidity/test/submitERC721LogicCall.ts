...
async function runTest(opts: {
  ...
  for (let i = 0; i < numTxs; i++) {
    await testERC721.approve(gravityERC721.address, 1 + i);
  ...
  let logicCallArgs = {
    ...
    payload: gravityERC721.interface.encodeFunctionData("withdrawERC721", [
      testERC721.address,
      tokenIds,
      destinations,
    ]), // payloads
  ...
  for (let i = 0; i < numTxs; i++) {
    expect(await testERC721.ownerOf(tokenIds[i])).to.equal(
      signers[i + 5].address
    );
  }
  ...
  expect(
    (await testERC20.balanceOf(logicCallSubmitResult.from)).toNumber()
  ).to.equal(9010);

  expect(
    (await testERC20.balanceOf(await signers[20].getAddress())).toNumber()
  ).to.equal(0);

  expect((await testERC20.balanceOf(gravity.address)).toNumber()).to.equal(990);

  expect(
    (await testERC20.balanceOf(ERC721LogicContract.address)).toNumber()
  ).to.equal(0);
