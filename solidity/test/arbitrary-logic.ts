...
import {
  SimpleLogicBatchMiddleware__factory,
...
async function runTest(opts: {
  ...
  const logicBatch = await new SimpleLogicBatchMiddleware__factory(
    signers[0]
  ).deploy();
  ...
  const logicContract = await new TestLogicContract__factory(signers[0]).deploy(
    testERC20.address
  );
  ...
  let fromBalance = await testERC20.balanceOf(await signers[0].getAddress());
  ...
  expect(await testERC20.balanceOf(logicCallSubmitResult.from)).to.equal(
    fromBalance.add("10")
  );

  expect(await testERC20.balanceOf(await signers[20].getAddress())).to.equal(
    40
  );

  expect(await testERC20.balanceOf(gravity.address)).to.equal(940);

  expect(await testERC20.balanceOf(logicContract.address)).to.equal(10);

  expect(await testERC20.balanceOf(await signers[0].getAddress())).to.equal(
    fromBalance.add("10")
  );
}
